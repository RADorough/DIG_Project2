...
var actorChars = {
  "@": Player,
  "o": Flag, // It's a finish line flag
  "=": Lava,
  "|": Lava,
  "v": Lava,
  "#": Lava,
...
function Lava(pos, ch) {
  ...
  if (ch == "=") {
    ...
    //Fast Horizontal spike
    this.speed = new Vector(-7,0);
    //this.repeatPos = pos; decided against having them reset the position
  } else if (ch == "<") {
    this.speed = new Vector(-11,0);
    this.repeatPos = pos; //FAST BOI
  }
...
function Barrel(pos){
  this.basePos = this.pos = pos.plus(new Vector(0,0.8));
...
Player.prototype.moveY = function(step, level, keys) {
  ...
  if (obstacle == "lava" && level.status == null) {
    level.status = "lost";
    level.finishDelay = 1;
    gravity = 33;
  }
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    ...
    // If the player hits the finish flag, player wins
